...
// Menangani pengiriman form untuk menyimpan mood
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Pastikan emoji dan mood level sudah dipilih
    $emoji = isset($_POST['emoji']) ? $_POST['emoji'] : '';
    $mood_level = isset($_POST['mood_level']) ? $_POST['mood_level'] : '';

    // Pastikan emoji dan mood level tidak kosong
    if ($emoji && $mood_level) {
        // Menyimpan data ke database
        $user_id = $_SESSION['user_id'];  // Pastikan session user_id sudah ada
        $query = "INSERT INTO mood (user_id, emoji, mood_level) VALUES (?, ?, ?)";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, 'isi', $user_id, $emoji, $mood_level);

        if (mysqli_stmt_execute($stmt)) {
            $message = "Mood Anda berhasil disimpan!";
        } else {
            $message = "Terjadi kesalahan. Coba lagi.";
        }
    } else {
        $message = "Silahkan pilih emoji dan mood level terlebih dahulu.";
    }
}
?>
...
<body>
    ...
    <?php if (isset($message)): ?>
        <script>
                    window.onload = function() {
                        ...
                        dialogBox.innerHTML = `<h1>INFORMASI</h1><p><?= $message ?></p><div class="dialog-buttons"><button class="dialog-button confirm" onclick="closeDialog()">OK</button></div>`;
    ...
    <!-- Form for selecting emoji and mood level -->
    <form method="POST">
        <div class="emoji-row">
        <div class="emoji">
    ...
    <div class="slider-label">
        ...
        <button class="button" type="submit">Simpan Emoji</button>
        </div>
    </form>
<script>
    // Mood slider event listener
    const moodSlider = document.getElementById('mood_slider');
    const moodLevelValue = document.getElementById('mood_level_value');
    
    moodSlider.addEventListener('input', () => {
        moodLevelValue.textContent = moodSlider.value;
    });

    // Display notifier after successful form submission
    <?php if (isset($message) && $message === "Mood Anda berhasil disimpan!"): ?>
        document.getElementById('notifier').style.display = 'block';
        setTimeout(() => { document.getElementById('notifier').style.display = 'none'; }, 3000);
    <?php endif; ?>
</script>
